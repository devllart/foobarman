...
$packages["devllart/foobarman/cmd/web_app"] = (function() {
	var $pkg = {}, $init, js, Drink, funcType, main;
	js = $packages["github.com/gopherjs/gopherjs/js"];
	Drink = $pkg.Drink = $newType(0, $kindStruct, "main.Drink", true, "devllart/foobarman/cmd/web_app", true, function(Name_) {
		this.$val = this;
		if (arguments.length === 0) {
			this.Name = "";
			return;
	...
	main = function() {
		$global.myButton.addEventListener($externalize("click", $String), $externalize((function() {
			$go((function() {
				var cmd, drink;
				cmd = $global.myInput.value;
				drink = new Drink.ptr($internalize(cmd, $String));
				$global.myContent.innerText = $externalize(drink.Name, $String);
			}), []);
		}), funcType));
	};
	Drink.init("", [{prop: "Name", name: "Name", embedded: false, exported: true, typ: $String, tag: ""}]);
	$init = function() {
		$pkg.$init = function() {};
		/* */ var $f, $c = false, $s = 0, $r; if (this !== undefined && this.$blk !== undefined) { $f = this; $c = true; $s = $f.$s; $r = $f.$r; } s: while (true) { switch ($s) { case 0:
		$r = js.$init(); /* */ $s = 1; case 1: if($c) { $c = false; $r = $r.$blk(); } if ($r && $r.$blk !== undefined) { break s; }
...
$synthesizeMethods();
$initAllLinknames();
var $mainPkg = $packages["devllart/foobarman/cmd/web_app"];
